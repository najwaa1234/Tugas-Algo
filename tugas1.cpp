...
void bacaFile(){
	...
	while(fscanf(file, " %[^;];%d;%d;%f\n", dataFilm[jumlahFilm].judul, &dataFilm[jumlahFilm].harga, &dataFilm[jumlahFilm].tahun, &dataFilm[jumlahFilm].rating) != EOF) {
		jumlahFilm++;
	}
	fclose(file);
}
...
void bubbleSortJudul(){
	for(int i=0; i < jumlahFilm-1; i++){
		...
		if(strcmp(dataFilm[j].judul, dataFilm[j+1].judul) > 0){
...
void linearSearchJudul(){
	...
	if(strcmp(dataFilm[i].judul, cari) == 0){
		found = true;
	}else{
		i++;
	}
...
void binarySearchJudul(){
	if(jumlahFilm < 30){
		cout << "[!] Jumlah film kurang dari 30. Gunakan Linear search." << endl;
		return;
	}
	...
	while(kiri <= kanan && !found){
		tengah = (kiri + kanan) / 2;
		int hasil = strcmp(dataFilm[tengah].judul, cari);
